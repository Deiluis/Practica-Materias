...
int cargarEnRegistro (void* reg, char* buffer) {
    ...
    *act = '\0';
    ...
    copiarString(r -> nGeneralAperturas, act +1, longitudString(act +1));
...
void mostrarRegistro (const void* reg) {
    ...
    printf(
        "%s;%s;%s\n",
        r -> periodo, r -> nGeneralAperturas, r -> indiceICC
    );
}
...
void corregirIccGeneralDef (Registro* reg) {
    ...
    secuenciaPalabrasCrear(&secL, reg -> nGeneralAperturas);
    secuenciaPalabrasCrear(&secE, reg -> nGeneralAperturas);
    ...
    while (!secuenciaPalabrasFin(&secL)) {
        ...
        if (nPalabra % 2 == 0)
            palabraModificar(&pal, revertirString);
        else
            palabraModificar(&pal, desencriptarIccGeneral);
    ...
    capitalizarString(reg -> nGeneralAperturas);
}

void corregirItemsObraDef (Registro* reg) {
    ...
    while (!secuenciaPalabrasFin(&secL)) {
        ...
        palabraModificar(&pal, capitalizarString);
...
char* desencriptarIccGeneral (char* str) {
